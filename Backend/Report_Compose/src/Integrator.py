...
class Integrator:
    ...
    async def queue_node(self, node_id: int, focus_message: str, dag: nx.DiGraph, mock: bool):
        """
        ...
        try:
            ...
            if mock:
                ...
                response, online_data = await self.process_node(node_id, focus_message)
                self.results_dag.store_result(node_id, {'llm_rep': response, 'online_data': online_data})
    ...
    async def generate_report(
        ...
        """
        dag = self.prompt_manager.prompt_dag
        logging.info(f"[Integrator Message] Generating report for {focus_message}")
        ...
        for node_id in nx.topological_sort(dag):
            self.tasks[node_id] = asyncio.create_task(self.queue_node(node_id, focus_message, dag, mock))
...
async def main():
        # Use a sample YAML file from your Prompts directory
        yaml_file_path = "./Prompts/Luis_Prompts.yaml"

        # Create an Integrator instance
        integrator = Integrator(yaml_file_path)

        # Define a focus message (you can use any string for testing)
        focus_message = "Testing mock integration"

        # Run the report generation in mock mode
        # This will simulate node processing and update the ResultsDAG accordingly.
        final_report_json = await integrator.generate_report(focus_message, mock=True)

        # Print the final JSON representation of the DAG results
